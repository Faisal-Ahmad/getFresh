...
module.exports = {
	...
	getByUn: function(user, callback){
		var sql = "select * from `product` where username=?";
	...
	orderCreate: function(user, callback){
		var sql = "insert into orders values (?,?,?,?,?)";
		db.execute(sql,['',user.id, user.name, user.description, user.price], function(status){
			callback(status);
		});
	},
	cartCreate: function(user, callback){
